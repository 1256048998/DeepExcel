...
| `tool_end` | `id`, `name`, `ok`, `duration_ms`, `summary?`, `error?`, `check?` | 与同 `id` 的 `tool_start` 配对。`summary` 是一句话结果（「20 行 × 4 列」）；`error` 是 `{code, message, hint?}`；`check` 是写后自动体检 `{ok, summary}`（新增公式错误、新增外部链接），面板只在 `ok: false` 时显示 |
